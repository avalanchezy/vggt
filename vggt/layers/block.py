...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor, pos=None, attn_mask=None) -> Tensor:
        ...
        def ffn_residual_func(x: Tensor, attn_mask=None) -> Tensor:
...
def drop_add_residual_stochastic_depth(
    ...
    if pos is not None:
        pos = pos[brange]
        attn_mask_subset = attn_mask[brange] if attn_mask is not None else None
        residual = residual_func(x_subset, pos=pos, attn_mask=attn_mask_subset)
    else:
        attn_mask_subset = attn_mask[brange] if attn_mask is not None else None
        residual = residual_func(x_subset, attn_mask=attn_mask_subset)
