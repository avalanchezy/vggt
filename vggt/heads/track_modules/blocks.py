...
class EfficientUpdateFormer(nn.Module):
    ...
    def forward(self, input_tensor, mask=None):
        ...
        if time_mask is not None:
            ...
            if expanded_time_mask.dtype == torch.bool:
                bool_mask = expanded_time_mask
                expanded_time_mask = torch.zeros(
                    bool_mask.shape, device=bool_mask.device, dtype=tokens.dtype
                )
                expanded_time_mask.masked_fill_(bool_mask, float("-inf"))
            else:
                expanded_time_mask = expanded_time_mask.to(dtype=tokens.dtype)
